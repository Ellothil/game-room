...
export function TicTacToeBoard({ roomId, isMyTurn }: TicTacToeBoardProps) {
  const user = useAuthStore((state) => state.user);
  const { board, currentPlayer, playerSymbol, winner, status } =
    useTicTacToeStore();
  ...
  return (
    <div className="flex flex-col items-center gap-6">
      <div className="text-center">
        ...
        <p className="font-medium text-xl">{getStatusMessage()}</p>
      </div>
        ...
        {board.map((_, index) => {
          ...
          return (
            <button
              ...
              disabled={
                !isMyTurn || status !== "playing" || Boolean(board[index])
              }
              key={cellKey}
